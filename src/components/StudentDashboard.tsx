...
import { BookOpen, Clock, CheckCircle, UserPlus } from "lucide-react";
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const fetchLectures = async () => {
    try {
      ...
      const enrolledLectureIds = new Set(
        enrollments?.map((e) => e.lecture_id) || []
      );
      const attendedLectureIds = new Set(
        attendanceRecords?.map((a) => a.lecture_id) || []
      );
  ...
  const handleEnrollInLecture = async (lectureId: string) => {
    try {
      ...
      setLectures((prev) =>
        prev.map((lecture) =>
          lecture.id === lectureId ? { ...lecture, is_enrolled: true } : lecture
        )
      );
    } catch (error: any) {
      if (error.code === "23505") {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600">
          Welcome back, {profile.full_name}! Track your attendance and manage
          your enrolled lectures.
        </p>
      </div>

      {/* Quick Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-[#D496A7] transition-all duration-200 transform  text-white rounded-xl p-6 shadow-lg">
        ...
        {enrolledLectures.length === 0 ? (
          <div className="bg-gray-50 rounded-xl p-8 text-center">
            ...
            <p className="text-sm text-gray-500">
              Browse available lectures above to get started
            </p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
